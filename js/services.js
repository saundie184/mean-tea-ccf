app.service('getTeaDataService', [getData]);

app.service('shoppingCartService', [updateCart]);
...
function updateCart(item) {
  ...
  this.updateItem = function(item, qty) {
    var index = cartArray.indexOf(item);
  ...
  this.getTotal = function() {
    var cartTotal = 0;
    //for every item in the cartArray, take the price times the quantity
    for (var i = 0; i <= cartArray.length - 1; i++) {
      var cost = cartArray[i].data.price;
      var qty = cartArray[i].quantity;
      var total = cost * qty;
  ...
  this.calculateBagSize = function() {
    // console.log(cartArray.length);
    var total = 0;
    for(var i = 0; i <= cartArray.length -1; i++){
      var qty = parseInt(cartArray[i].quantity);
      total = total + qty;
    }
    return total;
  };
